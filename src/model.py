...
class RNN_model(torch.nn.Module):
    ...
    def rnn_training(self, typed=False):
        ...
        loss_edit = 0
        loss_addingmotif = 0
        for k, target in enumerate(self.data['targets']):
            edges = edges_list[k]
            target_pred = []
            for j, transform in enumerate(target):
                ...
                if state in [4, 7]:
                    ...
                    loss = self.ce(motif_pred, motif_idx)
                    loss_list[k] += loss
                    # loss_addingmotif += loss

                    motif_emb = self.embedding_motif(motif_idx)
                    attachment_idx_pred = self.MLP_motif_attach_idx(torch.cat((rnn_vec, motif_emb), dim=1))
                    loss = self.ce(attachment_idx_pred, attachment_idx)
                    loss_list[k] += loss
                    # loss_addingmotif += loss
            ...
            total_loss += loss_list[k]

        # print("loss_edit: %.4f, loss_addingmotif: %.4f" % (loss_edit.item(), loss_addingmotif.item()))

        return total_loss, target_pred_res

    def forward(self, batch, typed=False, motif_vocab=None, motif_masks=None, beam_size=1, epoch=0, k=0, device=None):
        # if training mode
        if self.training or beam_size == 1:
            ...
            output = self.beam_deocde(
                ...
                motif_masks, batch.product, device, type=batch.type, typed=typed, k=k)
            if output != None:
